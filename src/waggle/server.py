...
def _run_admin_command(config: AppConfig, args: argparse.Namespace) -> int:
    ...
    if args.command == "create-api-key":
        ...
        )  # CLI output, intentionally bypasses logger
        return 0
    ...
    if args.command in {
        ...
        if args.command == "set-retention":
            ...
            print(json.dumps(_serialize_retention_policy(policy), indent=2))  # CLI output, intentionally bypasses logger
            return 0
        ...
        if args.command == "list-audit-events":
            ...
            print(json.dumps([_serialize_audit_event(event) for event in events], indent=2))  # CLI output, intentionally bypasses logger
            return 0
        runs = retention_backend.list_retention_runs(limit=getattr(args, "limit", 20))
        print(json.dumps([_serialize_retention_run(run) for run in runs], indent=2))  # CLI output, intentionally bypasses logger
        return 0
...
def _run_doctor(config: AppConfig, *, fix: bool = False, json_output: bool = False) -> int:
    ...
    if warnings:
        ...
        print(_c(_GREEN, f"All checks passed ({len(ok_items)} OK). Waggle looks healthy."))  # CLI output, intentionally bypasses logger
        print()  # CLI output, intentionally bypasses logger
        return 0
...
def _run_demo(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"  Imported {imported.nodes_created} nodes, {imported.edges_created} edges from demo.abhi")  # CLI output, intentionally bypasses logger
        print()  # CLI output, intentionally bypasses logger
        ...
        print(_c(_BOLD, "Query 2: What changed about the database decision?"))  # CLI output, intentionally bypasses logger
        result2 = graph.query(
        ...
        print(f"  (run 'WAGGLE_DB_PATH={demo_db} waggle-mcp ui' to open it)")  # CLI output, intentionally bypasses logger
        print()  # CLI output, intentionally bypasses logger
...
def _run_setup(args: argparse.Namespace) -> int:
    ...
    if args.run_doctor:
        ...
        if doctor_exit:
            print(_c(_CYAN, "Setup completed; doctor reported follow-up warnings above."))  # CLI output, intentionally bypasses logger
        return 0
    return 0
