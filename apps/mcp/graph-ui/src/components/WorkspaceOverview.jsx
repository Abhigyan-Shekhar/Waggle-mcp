...
export function WorkspaceOverview({
  ...
  const supersededIds = new Set(edges.filter((edge) => edge.relationship === "updates").map((edge) => edge.target_id));
  const authoritative = nodes.filter((node) => !node.valid_to && !supersededIds.has(node.id));
  const decisions = authoritative.filter((node) => node.node_type === "decision").slice(0, 5);
  ...
  return (
    <div className="workspace-page overview-canvas">
      ...
      <div className="context-decision-grid">
        <section className="workspace-panel context-panel" data-guide-focus="context">
          <div className="section-heading-inline">
            ...
            <span>{authoritative.length} memories · {decisions.length} decisions</span>
          </div>
      ...
      <div className="shared-memory-note">
        <ShieldCheck size={18} />
        <div><strong>Same memory. Same context.</strong><span>Both you and ChatGPT access this governed project memory through the same tools and rules.</span></div>
